...
from ollama_client import generate_text, chat_with_model, ollama_client, disable_thinking  
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/step2.out", mode="w", encoding="utf-8")  
    ]
)

err_handler = logging.FileHandler("logs/step2.err", mode="w", encoding="utf-8")
...
# Use only Ollama integration ---------------------------------------------------------

disable_thinking()
logger.info("Using Ollama as the only model calling backend")
OLLAMA_AVAILABLE = True
# ---------------------------------------------------------------------------

logger.info("Initializing basic GPU memory parameters (no longer loading local large models)")
...
def clear_memory():
    """Simplified GPU memory cleaning (only for Ollama client side)"""
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    gc.collect()
    monitor_gpu_memory()
...
def monitor_gpu_memory():
    """Print GPU memory status (if available)"""
    if torch.cuda.is_available():
        ...
        logger.info("CUDA not available, using CPU / remote inference")


def call_model(prompt, system_message=None, temperature=0.7, max_tokens=2000, task_type="literature_generation"):
    try:
        model_name = getattr(MODEL_CONFIG, 'main_model', 'qwen')
        temperature = float(min(max(temperature, 0.0), 1.0))
        max_tokens = int(min(max_tokens, 8192))
        system_prompt = system_message or getattr(
            SYSTEM_CONFIG, 'default_system_prompt', 'You are an expert academic writing assistant.')
        logger.debug(f"[Ollama] model={model_name} task={task_type} temp={temperature} max_tokens={max_tokens}")
        text = generate_text(
            prompt=prompt,
            model=model_name,
        ...
        logger.error(f"call_model invocation failed: {e}", exc_info=True)
...
def load_vector_store():
    ...
    if os.path.exists("./embeddings/faiss.index") and os.path.exists("./embeddings/metadata.npy"):
        ...
        try:
            ...
            if not os.path.exists("./converted_index/faiss.index"):
                logger.info("Copying FAISS index to compatible format")
                shutil.copy("./embeddings/faiss.index", "./converted_index/faiss.index")
            
            # Debug: Check copied index size
            copied_index = faiss.read_index("./converted_index/faiss.index")
            ...
            index = faiss.read_index("./converted_index/faiss.index")
        ...
        try:
            ...
            try:
                ...
                for section_name, content in report_data['sections'].items():
                    logger.info(f"Section '{section_name}' length: {len(str(content))} chars")
